...
[X.509 certificates](https://en.wikipedia.org/wiki/X.509) are basically wrappers around public keys with meta information that's interpreted in application specific contexts.
...
- [`hasIssuerDNProp()`](detail-dns.mdx#hasdnprop)
- [`getIssuerDNProps()`](detail-dns.mdx#getdnprops)
...
2. The start date. It's set to the current time by default.
   ...
   If you want to set your own start date you can do so by calling `$x509->setStartDate()`. That function has one required parameter - `$date` - which can be either an instance of `\DateTimeInterface` or a string. If it's a string it'll passed to the [constructor of `\DateTime`](https://www.php.net/manual/en/datetime.construct.php) and parsed by that.

3. The end date. It's set to one year from the current time by default.

   If you want to set your own start date you can do so by calling `$x509->setEndDate()`. That function has one required parameter - `$date` - which can be either an instance of `\DateTimeInterface` or a string.  If it's a string it'll be passed to the [constructor of `\DateTime`](https://www.php.net/manual/en/datetime.construct.php) and parsed by that.

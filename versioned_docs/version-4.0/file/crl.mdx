...
Of course, those fields are only of use if you _already_ have downloaded the CRL. So how do you store a CRL that's valid for ten days from the moment that you first downloaded it? That's where `X509::setCRLLookupCallback()` comes into play. Here's an example of its being used:

...
import GoodCSR from './inc/csr-good.md';
import EmptyCSR from './inc/csr-empty.md';
...
`$csr`, in this case, is a CSR object which, in turn, is basically a thin wrapper around [an instance of \phpseclib3\File\ASN1\Constructed](detail-constructed.mdx).  Passing `$csr` through `print_r()` or `var_dump()` triggers the [`__debugInfo()`](https://www.php.net/manual/en/language.oop5.magic.php#object.debuginfo) magic method which, in turn, produces the following output:
...
To learn more about the capabilities afforded to the CSR class by virtue of it being a thin wrapper around \phpseclib3\File\ASN1\Constructed please refer to [Deep Dive: ASN1\Constructed Objects](detail-constructed.mdx).
...
-----END PUBLIC KEY-----
```
Note that if the key is in an unsupported format then `$csr->getPublicKey()` will throw a `UnsupportedFormatException` exception. If you want to see what the public key is, regardless of whether or not it's in a supported format, then you can do `$csr['certificationRequestInfo']['subjectPKInfo']`.
...
Returns an array representing the first instance of a requested extension even if there are multiple instances. In this array `extnId` is a string, `critical` is a boolean and `extnValue` is an instance of `phpseclib3\File\ASN1\Types\BaseType`.
...
The subject DN can also be set by passing an instance of [`\phpseclib3\File\X509`](x509.mdx) to the CSR constructor. eg.
...
- **pkcs-9-at-challengePassword**
  ...
  `$csr->setChallengePassword()`. This function takes a single parameter - `$password` - which can be either a string or an instance of either `\phpseclib3\File\ASN1\Types\UTF8String` or `\phpseclib3\File\ASN1\Types\PrintableString`.
...
_Any_ attribute can be set by calling `$csr->setAttribute()`. `$x509->setAttribute()` has two required parameters - `$type` (a string) and `$value` (which can be any number of primitive types or object types).

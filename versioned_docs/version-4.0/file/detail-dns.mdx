...
In-so-far as strings go, `$props` can match the output of [ASN1::DN_STRING](#asn1dn_string):
...
In-so-far as arrays go, `$props` can match [ASN1::DN_OPENSSL](#asn1dn_openssl) or [ASN1::DN_ARRAY](#asn1dn_array).
...
In-so-far as `phpseclib3\File\ASN1\Element`'s go, `$props` can be anything (esp. if you're trying to create a malformed certificate for research purposes), however, a syntacticly valid `Element` instance should match [ASN1::DN_ASN1](#asn1dn_asn1).

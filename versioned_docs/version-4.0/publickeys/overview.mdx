...
If a key can't be loaded either a `phpseclib3\Exception\NoKeyLoadedException` exception will be thrown or a `phpseclib3\Exception\PasswordNeededException` will be thrown (if the key is encrypted and no password has been provided).

...
See [Introduction - Speed § Security Tradeoffs](intro/speed.md#security-tradeoffs) for a more in depth discussion.
...
$key = PublicKeyLoader::load(file_get_contents('/path/to/key.pem'), $password = null);
```
...
If a key can't be loaded either a `phpseclib4\Exception\NoKeyLoadedException` exception will be thrown or a `phpseclib4\Exception\PasswordNeededException` will be thrown (if the key is encrypted and no password has been provided).
...
All successfully loaded keys are an instance of `phpseclib4\Crypt\Common\AsymmetricKey`.
...
```php
echo $key instanceof \phpseclib4\Crypt\Common\PublicKey ? 'public key' : 'private key';
echo "\n";
echo $key instanceof \phpseclib4\Crypt\Common\PrivateKey ? 'private key' : 'public key';
```
`PrivateKey` is an interface implementing the following methods:

- `sign(string|Signable $source): string`
- `getPublicKey(): PublicKey`
- `toString(string $type, array $options = []): array|string`
- `withPassword(?string $string = null): PrivateKey`

The `string|Signable` parameter on `sign()` is discussed in the [Signable interface](#signable-interface) section.
...
- `toString(string $type, array $options = []): array|string`
- `getFingerprint(string $algorithm = 'sha256'): string`
...
Consider this hex encoded [twice signed CMS](/cms/signed.mdx):
...
The yellow highlighted portion is the signature subject and the blue highlighted portion is the signature, itself.

Resigning this would be rather difficult if you could only pass strings to the `sign()` method. Like you'd have to write a bunch of logic to do a `substr()` and then you'd have to know where to place the signature. If, in contrast, you could pass an _object_ to the `sign()` method this object could tell the `sign()` method where the signature subject could be found and where to place the signature. Such objects need to implement the `phpseclib4\File\Common\Signable` interface.
...
Supported key formats for a given algorithm can be determined by doing `\phpseclib4\Crypt\RSA::getSupportedKeyFormats()` (replace `RSA` with `DSA` or `EC` or whatever, as appropriate). What's returned is an associative array wherein the key is the name of the format and the value is the namespaced classname of the plugin that enables phpseclib to read the format.
...
```php
use phpseclib4\Crypt\Common\Formats\Keys\PuTTY;
...
OpenSSH private keys only support one encryption algorithm (aes256-ctr) so there are no methods to change it.
...
Custom key formats can be added by doing `\phpseclib4\Crypt\RSA::addFileFormat()`. So let's say you wanted to be able to load keys created by [PEAR's Crypt_RSA](https://pear.php.net/package/Crypt_RSA). The following would be sufficient:
...
## Comments
Only PuTTY and OpenSSH formatted keys support comments. See [Saving Keys](overview.mdx#saving-keys) for information on saving keys.

...
Note that multi-prime RSA has limited interoperability: most tools (OpenSSL, most CAs, most SSH and TLS implementations) only handle 2-prime RSA. Use `setSmallestPrime` with a value smaller than 4096 only when you control both sides of the exchange.
...
The [Probabilistic Signature Scheme](https://en.wikipedia.org/wiki/Probabilistic_signature_scheme). <span class="default">This is the default method.</span> It is more secure than the other methods but is less commonly used.
...
[Optimal Asymmetric Encryption Padding](https://en.wikipedia.org/wiki/Optimal_asymmetric_encryption_padding). <span class="default">This is the default method.</span> It is more secure than the other methods but is less commonly used.

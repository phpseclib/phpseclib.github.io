...
Structurally, the only hard requirement that distinguishes `EnvelopedData` from `EncryptedData` is that `EnvelopedData` requires at least one `RecipientInfo`:

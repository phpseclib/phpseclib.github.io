...
`$cms`, in this case, is a `CMS\EncryptedData` object which, in turn, is basically a thin wrapper around [an instance of \phpseclib4\File\ASN1\Constructed](file/detail-constructed.mdx).  Passing `$cms` through `print_r()` or `var_dump()` triggers the [`__debugInfo()`](https://www.php.net/manual/en/language.oop5.magic.php#object.debuginfo) magic method which, in turn, produces the following output:
...
```
If you don't know the content encryption key but are able to derive it with another key, be it a symmetric key or a private key (an instance of `\phpseclib4\Crypt\Common\PrivateKey`), you can do so thusly:

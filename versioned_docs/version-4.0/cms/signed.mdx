...
import SignedData from './inc/signeddata.md';
import ListAttributes from './inc/signeddata-listattributes.md';
import EmptySignedData from './inc/signeddata-empty.md';

One of the features the [Cryptographic Message Syntax (CMS)](https://en.wikipedia.org/wiki/Cryptographic_Message_Syntax) provides is to act as a wrapper format that bundles files and [digital sigantures](https://en.wikipedia.org/wiki/Digital_signature) of those files together.
...
`$cms`, in this case, is a `CMS\SignedData` object which, in turn, is basically a thin wrapper around [an instance of \phpseclib3\File\ASN1\Constructed](file/detail-constructed.mdx).  Passing `$cms` through `print_r()` or `var_dump()` triggers the [`__debugInfo()`](https://www.php.net/manual/en/language.oop5.magic.php#object.debuginfo) magic method which, in turn, produces the following output:
...
`getSigners()` returns an array of Signer objects and `findSigner()` returns either a single Signer object or null. The full namespace for the Signer object is `\phpseclib3\File\CMS\SignedData\Signer`.
...
```php
$x509 = \phpseclib3\File\X509::load('...');
...
$pfx->sign($cms);
```
`$pfx`, in this case, is an instance of [\phpseclib3\File\PFX](/file/pfx.mdx).
...
$private->sign($signer);
```
Where `$x509` and `$private` could correspond to `$pfx->getCertificates()[0]` and `$pfx->getPrivateKeys()[0]`, respectively, or just indepentendly loaded / generated certificates and keys (well, independent of a PFX - not of each other).
...
$private->sign($cms);
```
Where `$private` is an instance of `\phpseclib3\Crypt\Common\PrivateKey`.
...
- id-signingTime (which isn't required but, none-the-less, phpseclib adds it)
...
```
`validateSignature()` will also return false if the key usage extension isn't set appropriately (eg. if neither `digitalSignature` or `nonRepudiation` are set), `findSigner()` will fail. This key usage check can be disabled by doing calling `X509::ignoreKeyUsage()`.

...
If one does `echo base64_decode($x509->getDN(X509::DN_ASN1))` one would get this:
...
This one is similar to `X509::DN_ASN1` with the following differences:
...
To visualize the differences let's first do `echo base64_decode($x509->getDN(X509::DN_CANON))`:
...
Returns a string. More specifically, `X509::DN_HASH` returns the first 32-bits of the sha1 hash of the X509::DN_CANON output.
...
In-so-far as strings go, `$props` can can match the output of [X509::DN_STRING](#x509dn_string):
...
As discussed in [DN String Format Comparison](#dn-string-format-comparison) there are several different standards for representing DNs as strings. The above (and indeed X509::DN_STRING) corresponds to the output that _OpenSSL 3.0 CLI_ gives, however, the output that _PHP's openssl_x509_parse() with OpenSSL 3.0_ gives is also supported:
...
In-so-far as arrays go, `$props` can match [X509::DN_OPENSSL](#x509dn_openssl) or [X509::DN_ARRAY](#x509dn_array).

An example with X509::DN_OPENSSL:
...
In-so-far as `phpseclib3\File\ASN1\Element`'s go, `$props` can be anything (esp. if you're trying to create a malformed certificate for research purposes), however, a syntacticly valid `Element` instance should match [X509::DN_ASN1](#x509dn_asn1).

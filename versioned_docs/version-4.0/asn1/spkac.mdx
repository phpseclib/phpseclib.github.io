---
title: SPKAC
---

import GoodSPKAC from './inc/spkac-good.md';
import EmptySPKAC from './inc/spkac-empty.md';

SPKAC is an acronym for [Signed Public Key and Challenge](https://en.wikipedia.org/wiki/SPKAC), a lesser used CSR format without distinguished names or extensions.

## Reading SPKACs

```php
use phpseclib3\File\SPKAC;

$spkac = SPKAC::load(file_get_contents('spkac.txt'));

print_r($spkac);
```
<sup>([download spkac.txt](pathname:///asn1/spkac.txt))</sup>

`$spkac`, in this case, is a SPKAC object which, in turn, is basically a thin wrapper around [an instance of \phpseclib3\File\ASN1\Constructed](detail-constructed.mdx).  Passing `$spkac` through `print_r()` or `var_dump()` triggers the [`__debugInfo()`](https://www.php.net/manual/en/language.oop5.magic.php#object.debuginfo) magic method which, in turn, produces the following output:

<GoodSPKAC />
All elements and subelements of the above SPKAC object can be accessed as array elements vis-a-vis [ArrayAccess](https://www.php.net/manual/en/class.arrayaccess.php). As a consequence of this there are two ways to get (for example) the public key. You could do `$spkac['publicKeyAndChallenge']['spki']` or you could do `$spkac->getPublicKey()`. It's these helper functions that set the CSR class apart from \phpseclib3\File\ASN1\Constructed.

To learn more about the capabilities afforded to the SPKAC class by virtue of it being a thin wrapper around \phpseclib3\File\ASN1\Constructed please refer to [Deep Dive: ASN1\Constructed Objects](detail-constructed.mdx).

### getPublicKey()

```php
echo $spkac->getPublicKey();
```
Returns a `\phpseclib3\Crypt\Common\PublicKey` object that, by default, gets cast to a PKCS8-encoded public key:

```
-----BEGIN PUBLIC KEY-----
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCM00BBEr4iRFui8QRALkTZ/yJj
TbVsemyekfIYlIsmqolpgkhXNkXv2RNSkM8yWGS7+45YEo2Vb3X98/2z+8j4b24c
eB5g0Z6B/RXc6EpvHnX1GYMYofsfjP2U3is8qYWvuzPvmc7xD+QOb6wF5p9FXSOG
jXmnuljjPaeLwAF3AwIDAQAB
-----END PUBLIC KEY-----
```
Note that if the key is in an unsupported format then `$spkac->getPublicKey()` will throw a `UnsupportedFormatException` exception. If you want to see what the public key is, regardless of whether or not it's in a supported format, then you can do `$csr['publicKeyAndChallenge']['spki']`.

Very rarely, SPKACs may not have a public key at all. You can test for this by doing `$spkac->hasPublicKey()`, which returns a boolean based on whether or not the SPKAC in question has a public key or not.

### getChallenge()

`$spkac->getChallenge()` is a shorthand way of getting `$spkac['publicKeyAndChallenge']['challenge']`.










## Creating SPKACs

### Minimalistic SPKAC

Consider the following:

```php
use phpseclib3\File\SPKAC;

$spkac = new SPKAC();
echo $spkac;
```
The SPKAC that that would produce would look something like this:

```
SPKAC=MBYwDDAIMAMGAQADAQAWADADBgEAAwEA
```
This SPKAC has no public key, no challenge and no signature.

If you did `print_r($spkac)` on that SPKAC you'd get this:

<EmptySPKAC />

### Setting the Public Key

Let's say you created your public key thusly:

```php
use phpseclib3\Crypt\EC;

$private = EC::createKey('nistp256');
$public = $private->getPublicKey();
```
At this point you could set the public key for the SPKAC using one of two techniques:

```php
$spkac = new SPKAC($public);
echo $spkac;
```

```php
$spkac = new SPKAC();
$spkac->setPublicKey($public);
echo $spkac;
```

You can also remove a public key be calling `$spkac->removePublicKey()`.

### Setting the Challenge

The challenge can be set by calling `$spkac->setChallenge()` and passing a string to that function. It can also be set by setting `$spkac['publicKeyAndChallenge']['challenge']` to a string.

### Signing

Let's say you created your public key thusly:

```php
use phpseclib3\Crypt\EC;

$private = EC::createKey('nistp256');
$public = $private->getPublicKey();
```

At that point you can sign your SPKAC like so:

```php
$spkac = new SPKAC($public);
// do stuff
$private->sign($spkac);
echo $spkac;
```

## Validating Signatures

SPKAC's are always self-signed and can be validated thusly:

```php
$spkac = SPKAC::load('...');
echo $spkac->validateSignature() ? 'valid' : 'invalid';
```
...
<sup style="color: red"><strong>[5]</strong></sup> Private keys conform to the format described in [PROTOCOL.key](https://cvsweb.openbsd.org/cgi-bin/cvsweb/~checkout~/src/usr.bin/ssh/PROTOCOL.key?rev=1.1). Public keys are of the same format utilized by `~/.ssh/authorized_keys` and conform to [RFC4253](https://tools.ietf.org/html/rfc4253#page-15) / [RFC5656](https://tools.ietf.org/html/rfc5656#section-3.1).
